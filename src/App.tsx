...
import { Alarm, ArrowLeft, CaretRight, Check, CheckCircle, Clock, DotsThree, Fire, Gear, House, ListPlus, Pause, Play, Plus, Trash, TrendUp, X } from '@phosphor-icons/react'
...
export default function App() {
  ...
  const add = (title: string, note: string, makePrimary: boolean) => { setTasks(items => [...items.map(t => makePrimary ? { ...t, primary: false } : t), { id: crypto.randomUUID(), title, note, primary: makePrimary || items.length === 0, completed: false }]); setShowForm(false); setNotice('Tarea creada') }
  return <BrowserRouter><div className="app-shell">
    <AnimatePresence mode="wait">{focusTask ? <FocusScreen key="focus" task={focusTask} onClose={() => setFocusTask(null)} onComplete={() => { toggle(focusTask.id); setFocusTask(null); setNotice('Sesión completada · +25 min') }} /> : <motion.div key="today" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <main className="content"><section className="hello"><div><span className="lime-dot" /> <span>Tu ritmo de hoy</span></div><p>Un paso enfocado<br /><em>a la vez.</em></p></section>
        ...
        <section className="progress-card"><div className="card-top"><div><span className="label">PROGRESO DEL DÍA</span><h3>{percent === 100 ? 'Día completo' : 'Construyendo impulso'}</h3></div><div className="progress-number">{percent}<small>%</small></div></div><div className="progress-track"><motion.div className="progress-fill" animate={{ width: `${percent}%` }} /></div><div className="progress-meta"><span>{completed} de {tasks.length} tareas</span><span><Alarm size={15} /> 0 sesiones</span></div></section>
      </main><BottomNav /></motion.div>}</AnimatePresence>
    {showForm && <TaskForm onClose={() => setShowForm(false)} onSave={add} />}{notice && <div className="toast" role="status"><CheckCircle size={19} weight="fill" /> {notice}</div>}
  </div></BrowserRouter>
}
...
function TaskForm({ onClose, onSave }: { onClose: () => void; onSave: (title: string, note: string, primary: boolean) => void }) { const [title, setTitle] = useState(''); const [note, setNote] = useState(''); const [primary, setPrimary] = useState(false); const [error, setError] = useState(''); const save = () => { const result = taskSchema.safeParse({ title, note }); if (!result.success) return setError(result.error.issues[0]?.message ?? 'Revisa el título'); onSave(title.trim(), note.trim(), primary) }; return <div className="overlay" onMouseDown={onClose}><motion.div initial={{ y: 35 }} animate={{ y: 0 }} className="sheet" onMouseDown={e => e.stopPropagation()}><div className="sheet-head"><div><span className="eyebrow">NUEVA TAREA</span><h2>¿Qué quieres lograr?</h2></div><button className="icon-button" onClick={onClose} aria-label="Cerrar"><X size={21} /></button></div><label>Título<input autoFocus value={title} onChange={e => { setTitle(e.target.value); setError('') }} placeholder="Ej. Leer capítulo 3" />{error && <small className="error">{error}</small>}</label><label>Nota <span className="optional">OPCIONAL</span><textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Un poco de contexto…" /></label><button className="primary-button" onClick={save}><Check size={18} /> Guardar tarea</button><button className="primary-toggle" onClick={() => setPrimary(!primary)}><span className={`toggle ${primary ? 'on' : ''}`}><span /></span> Marcar como foco principal</button></motion.div></div> }

function FocusScreen({ task, onClose, onComplete }: { task: Task; onClose: () => void; onComplete: () => void }) { const [seconds, setSeconds] = useState(1500); const [running, setRunning] = useState(false); useEffect(() => { if (!running) return; const id = window.setInterval(() => setSeconds(s => Math.max(0, s - 1)), 1000); return () => window.clearInterval(id) }, [running]); const mins = String(Math.floor(seconds / 60)).padStart(2, '0'); const secs = String(seconds % 60).padStart(2, '0'); const finished = seconds === 0; return <motion.div className="focus-screen" initial={{ opacity: 0 }} animate={{ opacity: 1 }}><button className="close-focus" onClick={onClose} aria-label="Cerrar"><ArrowLeft size={21} /></button><div className="focus-center"><div className="focus-orbit"><div className="focus-core"><Clock size={27} weight="fill" /></div></div><span className="eyebrow">SESIÓN DE CONCENTRACIÓN</span><div className="big-time">{mins}:{secs}</div><p>{task.title}</p></div><div className="focus-panel"><span className="label">BLOQUE 01 · 25 MINUTOS</span><h2>{finished ? 'Buen trabajo. El bloque terminó.' : 'Tu atención es el espacio.'}</h2><div className="focus-actions"><button className="play-focus" onClick={() => finished ? onComplete() : setRunning(!running)}>{running ? <Pause size={19} weight="fill" /> : <Play size={19} weight="fill" />} {running ? 'Pausar' : finished ? 'Guardar sesión' : 'Comenzar'}</button><button className="finish-focus" onClick={onComplete}>Terminar</button></div></div></motion.div> }
